...
- **Trending** — open events ranked by 24h volume, refreshed every minute, with KPIs, generated
...
- **My Interests** — follow topics (popular tags plus every tag seen in the live feed, searchable).
  The app fetches each followed tag's top events and runs the same analysis on your personal feed.
...
| Endpoint | Used for |
|---|---|
| `gamma-api.polymarket.com/events?active=true&closed=false&order=volume24hr&tag_slug=…` | trending + per-topic feeds |

...
afterward happens locally — no incident text is ever sent to a server. No build step, no backend, no
API keys. It's plain HTML/CSS/JS (`index.html`, `style.css`, `app.js`).
...
Educational demo only, not legal advice. Explanations are generated by a small open-source language
model and can be wrong, oversimplified, or generic.
